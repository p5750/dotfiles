...
- `rules/base.rules` - Execution policy rules (command deny list, tracked in git)
- `.gitignore` - Ignores runtime-generated files (sessions, caches, DB, etc.)
...
Location: `config/codex/rules/base.rules`

Codex CLI loads all `*.rules` files from `CODEX_HOME/rules/` at startup. Base rules are tracked as `base.rules`; `default.rules` (auto-generated by Codex) is gitignored.
...
codex execpolicy check --pretty \
  --rules ~/.config/codex/rules/base.rules \
  -- <command>
```
